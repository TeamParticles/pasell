...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  Stream<ProductState> mapEventToState(ProductEvent event) async* {
    if (event is AddOrDeleteProductFavorite) {
      ...
      yield state.copyWith(products: product, total: sum, amount: product.length);
  ...
  Stream<ProductState> mapEventAddOrDeleteFavorite(
      String uidProduct) async* {
    try {
      final resp = await dbProductController.addOrDeleteCourseFavorite(
