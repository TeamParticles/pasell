...
class AdminProductBloc extends Bloc<AdminProductEvent, AdminProductstate> {
  ...
  Stream<AdminProductstate> _mapAddProduct(Product product) async* {
    try {
      ...
      final data = await adminController.addCourse(product: product);
  ...
  Stream<AdminProductstate> _mapUpdateProduct(Course course) async* {
    try {
      yield LoadingUpdateCoursestate();
      await Future.delayed(Duration(seconds: 2));

      final data = await adminController.updateCourse(product: product);
