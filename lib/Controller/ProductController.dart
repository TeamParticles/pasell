...
import 'package:pasell/Models/Home/userwshome.dart';
...
class ProductController {
  ...
  Future<List<Users>> getuserid({String id}) async {
    // final token = await authController.readToken();

    final response = await http.get(
      Uri.parse('$server/getuserid/$id'),
      headers: {
        'Content-type': 'application/json',
      },
    );

    return UserHome.fromJson(jsonDecode(response.body)).user;
  }
  ...
  Future<List<Product>> getproductid({String id}) async {
    ...
    final response = await http.get(
      Uri.parse('$server/getproductid/$id'),
